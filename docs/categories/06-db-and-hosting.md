...
- ~~**`mammoth`**~~ — **removed 2026-06-29 (P6-10)** via `npm uninstall mammoth`. Was a declared-but-unused `.docx`→text/HTML parser intended for a Word-upload path that never shipped. Re-add it only if/when `.docx` upload is actually built.
...
| ~~**P6-4**~~ | 🟢 | ✅ **RESOLVED (2026-06-29).** App uses **zero GraphQL** (all reads are REST `.from()` via the browser anon client → `authenticated` role). Two-part fix: **(1)** revoked `USAGE` on schemas `graphql`/`graphql_public` from anon/authenticated → the GraphQL endpoint is now **closed** for API roles (migration `cat6_p6_4_lock_down_unused_graphql`); **(2)** revoked `SELECT` on `trial_claims` from anon/authenticated → cleared its 0026 + 0027 findings (migration `cat6_p6_4_revoke_trial_claims_select`). **Accepted by necessity:** `profiles` + `usage_events` keep their `authenticated SELECT` grant because the app's REST reads (AllowanceBar/PlanNotice/SettingsPanel/`lib/usage`) require it — revoking would break the app. Their residual 0027 lint is a **static SELECT-grant heuristic**, not a live exposure: RLS still gates every row, and the GraphQL endpoint is disabled regardless. ⚠️ **Lesson:** 0026/0027 key off the table `SELECT` grant, *not* GraphQL schema `USAGE` — disabling GraphQL hardens the surface but does not clear the lint. | Done (2 tables accepted; see note). |
...
| **P6-9** | 🟠 | **No staging/prod separation** — migrations (incl. via MCP) hit prod directly; a bad change has no buffer. **DEFERRED (2026-06-29):** a persistent Supabase branch costs ~$9.80/mo — not worth it pre-revenue. Safety net for now = small reviewed changes + exported migrations (P6-8) + backups (P6-11) + advisor re-runs. **Plan: spin up an on-demand branch ($0 at rest, ~cents/use) only when a change is risky; revisit a standing staging DB once paying customers exist.** | On-demand branch when needed; persistent staging later. |
| ~~**P6-10**~~ | 🟢 | ✅ **DONE — DROPPED (2026-06-29).** `mammoth` (`.docx`→text/HTML parser) was declared but imported nowhere. It was meant to add a Word `.docx` upload path (`mammoth.extractRawText` → the existing `memoText`/`pages` text pipeline; the deleted `UploadZone`/P3-3 shared that ambition), but the marking flow is PDF-only (text extracted client-side via `pdfjs-dist`). Removed via `npm uninstall mammoth`. **If `.docx` upload is ever wanted, re-add it then** — it's a real feature, just not worth a stranded dependency now. | Done (removed). |
