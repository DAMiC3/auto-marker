...
### Problem 7 — batch estimate can undershoot real cost (P1-4) ✅ BUILT (2026-06-16)
...
**As built (2026-06-16) — what shipped, and the one simplification:**
- **Sizing oracle, server-side (C15, privacy).** `affordableDocCount(papers, remainingZar, quality)` + `MAX_BATCH_DOCS = 100` in `lib/usage.ts`. Returns how many of the leading documents fit `remaining` by the conservative estimate, capped at 100; `0` = stop. All Rand maths stay server-side — the client only ever receives this integer (never a Rand figure).
- **Batch route** (`app/api/mark/batch/route.ts`): rejects any submission over `MAX_BATCH_DOCS` (`batch_too_large`, C15); the over-budget branch returns `402 { affordable }` from `affordableDocCount`; the GET poll now returns `recorded: boolean` so the loop can honour C6.
- **Over-limit dialog** (`app/page.tsx`): on the initial probe returning `402 { affordable > 0 }`, a modal offers **Mark in chunks** / **Remove some documents** (the latter cancels with nothing moved or marked). Shows **document count only**. `affordable === 0` → straight limit-reached, no dialog.
- **The loop** (`startChunkLoop` in `app/page.tsx`): runs automatically — probe `submitWithRetry(remaining)` → if over, submit exactly `affordable` → `pollBatch` (awaits completion **and** the `recorded` flag) → stamp/move → advance the done-pointer → repeat. Enforces C5 (strict sequencing), C6 (stop if `recorded === false`), C7 (pointer advances only after a chunk is applied), C10 (`busy`/`chunkCtx` single-flight lock + `canMark` disabled), C12 (`submitWithRetry`, ≤2 retries, never retries `FATAL_RUN_ERRORS`), C13 (`verification_failed` → distinct banner), C3 (`maxIterations = docCount + 1` backstop). Reuses the route's `affordable` rather than re-deriving sizing (C15).
- **⚠️ Simplification vs the agreed spec — the explicit R10 floor was NOT built.** Chunks are sized purely by `affordableDocCount` (whole docs that fit in remaining, ≤100). Shrinking happens *naturally* because `remaining` falls each chunk; the loop stops when not even one more document fits. With document-granular sizing the loop already (a) terminates, (b) keeps round-trips bounded (≤ docCount), and (c) keeps the cap-crossing chunk small *because near the limit `remaining` is small*, so a separate `MIN_CHUNK_ZAR = 10` constant and deliberate shrink-curve were unnecessary. **Trade-off:** an estimate miss on an *early* large chunk (when remaining is still large but the 100-doc cap doesn't bind) could write off more than R10. If that shows up in real data, add an explicit per-chunk Rand cap then. Documented so the choice is visible, not silent.
- **Still TODO (small):** page-ops on an actual write-off (a chunk that crosses the cap) — currently the overspend is simply absorbed; wiring `notifyOps` there would let you see real money eaten. And recalibrating the token constants from real `usage_events` (keep them conservative — C16).

**Update (2026-07-18) — the chunk loop now also handles a PDF-COUNT overload, not just an allowance one.** Before this, a batch of **>100 PDFs** hit the server's `batch_too_large` (C15) on the first probe and surfaced as the *generic* error (and paged ops) — the chunk machinery was only reachable via an allowance `402`. With the Moodle zip ingestion (Cat 2 §6.2b) making 100+ document jobs common — and batch being the mode users want for a whole class — that path is now graceful:
- **Entry** (`runBatch`): `prepared.length > MAX_BATCH_DOCS` routes straight to the same "Mark in chunks" dialog — **no whole-job probe** (a probe that "fit" would create a first-slice batch we'd then have to discard/pay for). `ChunkCtx.reason` = `"count"` (>100, must split regardless of budget) or `"budget"` (≤100 but over allowance); the dialog title/body and secondary-button label adapt (count → *"This batch has more than 100 papers"* / *"Cancel"*).
- **Sizing** (`startChunkLoop`): probes `remaining.slice(0, want)` where `want = min(MAX_BATCH_DOCS, remaining.length)`, and `chunkSize = min(want, affordable)` on an over-budget result. So **every chunk respects BOTH limits** — ≤100 **and** ≤ what the budget covers — and no submission can ever exceed 100. Verified by simulation across count-only, budget-only, and combined overloads (e.g. 250 docs/unlimited → `[100,100,50]`; 250 docs/budget-130 → `[100,30]` + 120 left). All of C1–C16 still hold; the C3 backstop (`docCount + 1`) sits far above the real iteration count (`≈ ceil(docs/100)`).
- **Constant home:** `MAX_BATCH_DOCS` moved to `lib/markingPrompt.ts` (client+server-shared) so the client chunk loop can match the server ceiling **without** importing server-only `lib/usage.ts` (which pulls in the Supabase service client); `usage.ts` re-exports it for the batch route. The server `batch_too_large` guard (C15) stays as the hard backstop — the client cap just means a well-behaved client never trips it.
...
| **P1-4** | 🟢 | **Batch estimate can under-block** (Problem 7) — per-page token budgets are conservative but not guaranteed ceilings; a dense script can run higher and the batch overspends. | ✅ **BUILT + LIVE (2026-06-16).** Over-limit batch shows a dialog: **"Mark in chunks"** (automatic, self-correcting loop) or **"Remove some documents"** (cancel → start). Loop sizes each chunk via server-side `affordableDocCount` (≤100 docs that fit remaining), reconciles each against *actual* cost, advances only after recording, stops when nothing more fits or on a record/verify failure. Safety invariants C1–C16 enforced (§11b). **Documents-only UI, no Rand.** *Simplification:* explicit R10 floor not built — doc-granular sizing covers it (see §11b "As built"). |
