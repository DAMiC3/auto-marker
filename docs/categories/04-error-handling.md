...
| **Corrupt / password-protected PDF** | `preparePaper` (pdf.js) throws **inside each paper's own `try/catch`** (instant *and* batch) → recorded as "not marked", original left in From, **the run continues** (P2-3). No whole-run abort. | Message is generic ("not marked"); no pre-validation | Phase 4: validate before queueing, clearer per-file reason |
...
6. ~~**Instant-mode loop abort**~~ — ✅ **RESOLVED (P2-3).** `runInstant` wraps each paper in its own `try/catch`: a bad paper (corrupt/password PDF, parse error) is recorded "not marked", left in From, and the loop continues — matching batch. Only a plan/auth `FATAL_RUN_ERRORS` code aborts the whole run.
...
- **Liveness endpoint (2026-07-18):** `GET /api/health` returns `200 {ok,ts}`, dependency-free (no Supabase/Anthropic), bypassed in `middleware.ts` so it answers without an auth redirect. It exists so an **external** monitor (off-Cloudflare) can detect a full hosting/Worker outage — the one failure the in-Worker `notifyOps` can't alert on. **Owner action still required:** point a free external monitor (e.g. UptimeRobot) at it; until then the endpoint is unused.
- **Unload guard (2026-07-18):** `app/page.tsx` arms a `beforeunload` warning while `busy`, so closing the tab mid-run triggers the browser's native "Leave site?" prompt (mitigates the C11 closed-tab abandonment).
- **Still missing:** no exception tracking (Sentry), **no** Logpush sink (searchable history), **no external uptime monitor wired** (endpoint now exists), **no** admin dashboard, **no** metrics (error rate, latency, active users, daily spend), **no** cron drain of the D1 dead-letter buffer.
