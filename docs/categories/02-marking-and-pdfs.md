...
3. **If no text layer → it's a scan/diagram:**
   ...
   - **Blank check** (`isCanvasBlank`): samples ~1 pixel in every 16; if **< 0.05%** of sampled pixels carry ink, the page is treated as blank and **skipped entirely** (no tokens spent). The threshold is deliberately low so a faint real answer page isn't dropped (P2-2); skipped pages are **counted and surfaced** to the user in the result banner.
...
| ~~**P2-2**~~ | ✅ | ~~**Blank-page false-skip**~~ — **Resolved (both halves).** (1) The blank threshold was lowered **0.2% → 0.05%** ink (`isCanvasBlank`), so a faint/low-contrast real answer page is far less likely to be judged blank. (2) Skipped blanks are no longer silent: `preparePaper` **counts** them (`skippedBlank`), the count flows through `MarkOutcome`/`PreparedDoc`, and the result banner shows a **heads-up** ("N page(s) looked blank and were skipped — if a marked paper seems short, check the original") across instant, batch, and the chunk loop. | Done — `lib/markPaper.ts` `isCanvasBlank`/`preparePaper`, `app/page.tsx` `blankNote`/`finish`. |
