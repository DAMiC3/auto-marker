...
3. **If no text layer → it's a scan/diagram:**
   ...
   - **Blank check** (`isCanvasBlank`): samples ~1 pixel in every 16; if **< 0.05%** of sampled pixels carry ink, the page is treated as blank and **skipped entirely** (no tokens spent). The threshold is deliberately low so a faint real answer page isn't dropped (P2-2). Skips are silent by design — a user-facing "N skipped as blank" banner was tried and removed as too noisy.
...
- **Prompt-injection quarantine (P5-1).** Right after `preparePaper`, the client runs `hasFenceCollision(pages)` (from `lib/markingPrompt.ts`). A paper whose extracted text contains the `STUDENT_FENCE` marker is a wrapper-forgery attempt — it is **never sent to the model**. Its original is moved to a sibling **`"Problematic papers"`** folder (`getProblematicFolder`, get-or-create under the connected root) as **`"<name> (attempted prompt injection).pdf"`** via `uniqueName`, recorded as `quarantined`, and reported in the banner/Results. The reason is a parameter, so the same path can serve future rejection reasons. Works identically across instant, batch, and the chunk loop (quarantine happens at prep time, before submit). See Category 5 §3–4 for the fence + prompt side.
...
| ~~**P2-2**~~ | ✅ | ~~**Blank-page false-skip**~~ — **Resolved by threshold only.** The blank threshold was lowered **0.2% → 0.05%** ink (`isCanvasBlank`), so a faint/low-contrast real answer page is far less likely to be judged blank (a truly empty page still sits near 0%). A user-facing "N pages skipped as blank" banner was built and then **deliberately removed** — judged too noisy/irritating for the common case where the skipped pages really are blank. | Done — `lib/markPaper.ts` `isCanvasBlank`. |
...
| ~~**P2-4**~~ | ✅ | ~~**`max_tokens` truncation**~~ — **Resolved (16k accepted; chunking declined 2026-06-18).** Cap raised `4096 → MAX_OUTPUT_TOKENS = 16000` and both routes **detect `stop_reason === "max_tokens"`** and fail loudly (instant: 422; batch: per-paper error) instead of silently half-marking. 16k ≈ **400+ annotations** (2–4× a heavy typed university test), and **only output tokens count** — the memo/pages are input and don't eat the budget — so a realistic paper won't hit it. A rare overflow is *reported*, never silently corrupted, which is good enough. The page-range chunking idea was considered and **deliberately dropped** as unnecessary. (= P5-3) ⚠️ If thinking is ever enabled on the marking calls, thinking tokens count against the 16k — bump the cap if so. | Done — `lib/markingPrompt.ts`, `app/api/mark/route.ts`, `app/api/mark/batch/route.ts`. |
| ~~**P2-5**~~ | ✅ | ~~**Memory pressure**~~ — **Guarded.** `runBatch` still prepares all PDFs up front, but now **estimates the running payload during prep** (`preparedBytes` = original bytes + page strings) and **stops before the tab OOMs** once it crosses `BATCH_MEMORY_BUDGET` (~600 MB) — plus a `RangeError` catch for an allocation failure mid-prep. The user gets *"This batch is too large… mark fewer PDFs at a time"* and **nothing is submitted, charged, or moved** (prep runs before submit). For the real product — typed PDFs — 100 papers is only ~50–90 MB, so this never trips; it only fires on image-heavy/scanned batches (each page is a multi-MB base64 string), which is the unbuilt P2-9 territory. A true OOM crashes the tab with no catchable error, so the only robust fix is this pre-flight ceiling, not a try/catch around the crash. | Done — `app/page.tsx` `BATCH_MEMORY_BUDGET` / `preparedBytes` / `memoryGuardMessage` + `runBatch` prep loop. |
...
| **P2-7** | 🟡 | **Fixed extraction heuristics** — line-group (0.012) and blank (0.0005) thresholds misbehave on unusual layouts → misgrouped lines / misplaced `y` hints. | Tune or make adaptive. |
