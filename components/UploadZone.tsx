  ...
  return (
    <div
      ...
      className={`flex-1 flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed px-6 py-8 cursor-pointer transition-colors ${
        accent
          ? "border-[var(--accent-400)] bg-[var(--accent-50)]/50 hover:bg-[var(--accent-50)]"
    ...
    >
      ...
      <div
        className={`w-10 h-10 rounded-full flex items-center justify-center ${
          accent ? "bg-[var(--accent-100)]" : "bg-slate-200"
        }`}
      >
        <svg
          className={`w-5 h-5 ${accent ? "text-[var(--accent-600)]" : "text-slate-500"}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
      ...
      {file ? (
        <>
          <p className={`text-[14px] font-semibold text-center ${accent ? "text-[var(--accent-700)]" : "text-slate-700"}`}>
            {file.name}
          </p>
